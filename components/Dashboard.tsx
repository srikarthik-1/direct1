...
import type { Customer, HistoryEntry, Settings, Tier } from '../types';
import { AnalyticsCharts } from './AnalyticsCharts';
import { SettingsPage } from './SettingsPage';
import { 
  MenuIcon, PowerIcon, ChartLineIcon, PlusIcon, SearchIcon, UsersIcon, PieChartIcon, DownloadIcon,
  WalletIcon, CoinsIcon, ShoppingBagIcon, SettingsIcon, HistoryIcon, SparklesIcon
} from './Icons';
...
const TopBar: React.FC<{ onToggleSidebar: () => void, onLogout: () => void }> = React.memo(({ onToggleSidebar, onLogout }) => (
  <header className="fixed top-0 left-0 w-full h-16 bg-white/80 backdrop-blur-lg border-b border-neutral-200 flex justify-between items-center px-5 z-40">
    <div className="flex items-center gap-4">
      ...
      <div className="font-serif text-xl text-black">Loyalty <span className="italic">Pro</span></div>
    </div>
    <div className="flex items-center gap-4">
...
const StatCard: React.FC<{ label: string, value: string | number }> = ({ label, value }) => (
    <Card>
        ...
        <div className="text-4xl font-serif text-black">{value}</div>
    </Card>
);
...
const Dashboard: React.FC<DashboardProps> = ({ userType, onLogout, onSignIn }) => {
  ...
  const handleTransaction = useCallback((
    ...
    setCustomers(prev => {
        ...
        const tierDiscountAmount = (billAmount * tierDiscountPercent) / 100;
  ...
  const PageTitle: React.FC<{ children: React.ReactNode }> = ({ children }) => (
    <h1 className="text-4xl font-serif text-black mb-8 pb-6 border-b border-neutral-200">{children}</h1>
  );
      ...
      <main className={`pt-24 px-4 sm:px-8 pb-8 transition-all duration-300 ease-in-out ${isSidebarCollapsed ? 'ml-16' : 'ml-60'}`}>
        ...
        <Section id="analytics">
            <div className="flex justify-between items-end mb-8 pb-6 border-b border-neutral-200">
                <h1 className="text-4xl font-serif text-black">Intelligence Hub</h1>
...
const TransactionForm: React.FC<{customers: Customer[], onTransaction: Function, settings: Settings}> = ({customers, onTransaction, settings}) => {
    ...
    }, [billAmount, tierDiscountPercent]);
        ...
        <Card className="max-w-xl">
            <form onSubmit={handleSubmit} noValidate>
                ...
                <div className={`transition-all duration-500 ease-in-out overflow-hidden ${isBillingVisible ? 'max-h-[40rem] opacity-100 mt-6 pt-6 border-t border-dashed border-neutral-200' : 'max-h-0 opacity-0'}`}>
                    ...
                    {existingCustomer && existingCustomer.points > 0 && (
                        <div className="flex items-center justify-between bg-neutral-50 p-4 border border-neutral-200 rounded-md mb-6">
                           <label htmlFor="use-points-checkbox" className="flex-grow cursor-pointer">
                                <span className="font-semibold text-black">Use Points for Discount</span>
                                <p className="text-sm text-neutral-600">Available: <span className="font-bold">{existingCustomer.points.toLocaleString()}</span></p>
                           </label>
                           <input 
                    ...
                    <div className="space-y-2 text-center bg-neutral-50 p-4 border border-neutral-200 rounded-md">
                        {tierDiscountAmount > 0 && <p className="text-blue-600 text-sm">Tier Discount ({tierDiscountPercent}%): -₹{tierDiscountAmount.toFixed(2)}</p>}
                        {pointsToRedeem > 0 && <p className="text-red-500 text-sm">Points Redeemed: -₹{pointsToRedeem.toFixed(2)}</p>}
                        <p className="font-semibold text-black text-lg">Final Bill: ₹{finalBill.toFixed(2)}</p>
                        {pointsEarned > 0 && <p className="text-green-500 text-sm">Change converted to Points: +{pointsEarned.toLocaleString()}</p>}
                    </div>
                </div>
...
const AIAnalysisSection: React.FC<{ customers: Customer[] }> = ({ customers }) => {
    ...
    return (
        <Card>
            <h3 className="font-serif text-2xl text-black">AI-Powered Business Insights</h3>
            ...
            {analysisResult && (
                <div className="mt-6 pt-6 border-t border-neutral-200">
                    <h4 className="font-serif text-lg text-black mb-2">Analysis Result:</h4>
                    <div className="bg-neutral-50 p-4 border border-neutral-200 rounded-md">
